...
namespace TallerCaja.Services
{
    ...
    public class IntegracionService : IIntegracionService
    {
        ...
        {
            _http.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", SessionManager.Token);
            return await PostAsync<AbrirTurnoResponse>("/int/caja/turno/abrir", request);
        }
        public async Task<CerrarTurnoResponse?> CerrarTurnoAsync(CerrarTurnoRequest request)
        {
            _http.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", SessionManager.Token);
            return await PostAsync<CerrarTurnoResponse>("/int/caja/turno/cerrar", request);
        }

        public async Task<CobroResponse?> ProcesarCobroAsync(CobroRequest request)
        {
            _http.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", SessionManager.Token);
            return await PostAsync<CobroResponse>("/int/caja/cobro", request);
        }

        public async Task<FacturaPendienteDto> BuscarFacturaPendienteAsync(string? placa = null, string? numeroFactura = null)
        {
            _http.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", SessionManager.Token);
            var url = "/int/caja/facturas/buscar?";
            if (!string.IsNullOrEmpty(placa)) url += $"placa={placa}";
            else if (!string.IsNullOrEmpty(numeroFactura)) url += $"numeroFactura={numeroFactura}";
        ...
        {
            _http.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", SessionManager.Token);
            return await PostAsync<PagoFacturaResponse>($"/int/caja/facturas/{facturaId}/pago", request);
        }

        public async Task<AbonoResponse?> RegistrarAbonoAsync(int cxcId, AbonoRequest request)
        {
            _http.DefaultRequestHeaders.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", SessionManager.Token);
            return await PostAsync<AbonoResponse>($"/int/caja/cxc/{cxcId}/abono", request);
        }
